        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <el-row>
              ...
              <el-button size="mini" type="text" @click="handleSelectResource(scope.$index, scope.row)">
                分配资源
              </el-button>
            </el-row>
            <el-row>
...
<script>
  import {fetchList,createRole,update,deleteRole} from '@/api/role';
  ...
  export default {
    ...
    methods: {
      ...
      handleDialogConfirm() {
        this.$confirm('是否要确认?', '提示', {
          ...
          if (this.isEdit) {
            ...
            update(this.role.id,params).then(response => {
              this.$message({
                message: '修改成功！',
                type: 'success'
              });
      ...
      handleSelectMenu(index,row){
        this.$router.push({path:'/auth/allocMenu',query:{roleId:row.id}})
      },
      handleSelectResource(index,row){
        this.$router.push({path:'/auth/allocResource',query:{roleId:row.id}})
      },
      getList() {
